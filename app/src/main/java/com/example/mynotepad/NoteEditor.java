...
public class NoteEditor extends AppCompatActivity {
    ...
    private final String items[]={"其它","旅游","工作","学习","生活"};
            ...
            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                ...
                if(text.equals(date)||date==null){
                    if(hourOfDay<=calendar.get(Calendar.HOUR_OF_DAY))
                        if(minute-1<=calendar.get(Calendar.MINUTE)){
                            time=" "+calendar.get(Calendar.HOUR_OF_DAY)+":"+(calendar.get(Calendar.MINUTE)+1);
                        }
                        else{
